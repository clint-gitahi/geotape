...
import {editMeasurements} from '../actions/gpsActions';
...
function EditMeasurement({route, navigation}) {
  ...
  React.useEffect(() => {
    setMeasurementTxt(item.measurementTxt);
    setUserPoints(item.usersPoints);
  }, [item]);
  ...
  const handleSave = () => {
    ...
    dispatch(editMeasurements(item.measurementId, measurementTxt, usersPoints));
    navigation.goBack();
  };

  return (
    <SafeAreaView style={styles.container}>
      <Text style={styles.headerTxt}>Edit {item.measurementTxt}</Text>
